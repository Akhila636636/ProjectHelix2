...
st.caption("App version: FINAL MVP (Vertex-ready)")
...
def vertex_ai_tumor_segmentation(volume):
    """
    Simulated Vertex AI tumor segmentation.
    In production, this function calls a Vertex AI endpoint.
    """
    vol_norm = (volume - volume.min()) / (volume.max() - volume.min() + 1e-8)
    threshold = np.percentile(vol_norm, 99.2)
    tumor_mask = vol_norm > threshold
    return tumor_mask
...
if uploaded_file is not None:
    ...
    if np.any(tumor_mask_3d):
        ...
        st.warning("⚠️ No tumor-like region detected for 3D rendering.")
